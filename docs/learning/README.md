# Learning Documents

Each document explains a major engineering area in interview-friendly language:

1. `01-project-architecture.md`
2. `02-nextjs-full-stack-flow.md`
3. `03-supabase-authentication.md`
4. `04-postgresql-relational-design.md`
5. `05-row-level-security.md`
6. `06-environment-variables-and-secrets.md`
7. `07-git-repository-foundation.md`
8. `08-simple-ui-and-accessibility.md`
9. `09-pet-profile-crud.md`
10. `10-pet-avatar-storage.md`
11. `11-private-photo-upload.md`
